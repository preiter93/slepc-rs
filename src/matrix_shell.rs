...
impl PetscMat {
    ...
    /// // Initialize slepc
    /// let n = 100;
    /// let world = SlepcWorld::initialize().unwrap();
    ///
    /// // Set context
    /// let mut ctx: (i32, i32) = (n, n);
    ///
    /// // Initialize shell matrix
    /// let mat = PetscMat::create_shell(&world, None, None, Some(n), Some(n), Some(&mut ctx)).unwrap();
    ///
    /// // Get context
    /// let ctx_: (i32, i32) = mat.shell_get_context().unwrap().expect("No context found");
    ///
    /// assert_eq!(ctx, ctx_)
    /// ```
    ///
